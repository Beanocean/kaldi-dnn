// featbin/compute-spectralband-feats.cc
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Create spectralband feature files.\n"
        "Usage:  compute-spectralband-feats [options...] <wav-rspecifier> <feats-wspecifier>\n";

    // construct all the global objects
    ParseOptions po(usage);
    SpectralbandOptions spectralband_opts;
    ...
    // Register the spectralband option struct
    spectralband_opts.Register(&po);
    ...
    Spectralband spectralband(spectralband_opts);
    ...
    for (; !reader.Done(); reader.Next()) {
      ...
      if (spectralband_opts.frame_opts.samp_freq != wave_data.SampFreq())
        KALDI_ERR << "Sample frequency mismatch: you specified "
                  << spectralband_opts.frame_opts.samp_freq << " but data has "
      ...
      try {
        spectralband.Compute(waveform, vtln_warp_local, &features, NULL);
      } catch (...) {
      ...
      if (output_format == "kaldi") {
        ...
        HtkHeader header = {
          ...
          static_cast<uint16>( 006 | // spectralband
          (spectralband_opts.use_energy ? 0100 : 020000)) // energy; otherwise c0
        };
        p.second = header;
