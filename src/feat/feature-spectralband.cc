...
#include "feat/feature-spectralband.h"


namespace kaldi {

Spectralband::Spectralband(const SpectralbandOptions &opts)
...
const MelBanks *Spectralband::GetMelBanks(BaseFloat vtln_warp) {
...
void Spectralband::Compute(const VectorBase<BaseFloat> &wave,
                   BaseFloat vtln_warp,
                   Matrix<BaseFloat> *output,
                   Vector<BaseFloat> *wave_remainder) {
  KALDI_ASSERT(output != NULL);
  int32 rows_out = NumFrames(wave.Dim(), opts_.frame_opts);
     // cols_out = opts_.num_bins;
  const MelBanks *this_mel_banks = GetMelBanks(vtln_warp);
  int32 cols_out = this_mel_banks -> NumBins();
  ...
  for (int32 r = 0; r < rows_out; r++) {  // r is frame index..
    ...
    SubVector<BaseFloat> this_spectralband(output->Row(r));

    // this_spectralband = dct_matrix_ * mel_energies [which now have log]
    // this_spectralband.AddMatVec(1.0, dct_matrix_, kNoTrans, mel_energies, 0.0);
    this_spectralband.CopyFromVec(mel_energies);
    // if (opts_.cepstral_lifter != 0.0)
      // this_spectralband.MulElements(lifter_coeffs_);

    if (opts_.use_energy) {
      if (opts_.energy_floor > 0.0 && log_energy < log_energy_floor_)
        log_energy = log_energy_floor_;
      this_spectralband(0) = log_energy;
    }

    if (opts_.htk_compat) {
      BaseFloat energy = this_spectralband(0);
      for (int32 i = 0; i < cols_out-1; i++)
        this_spectralband(i) = this_spectralband(i+1);
      if (!opts_.use_energy)
        energy *= M_SQRT2;  // scale on C0 (actually removing scale
      // we previously added that's part of one common definition of
      // cosine transform.)
      this_spectralband(cols_out-1)  = energy;
    }
