...
#ifndef KALDI_FEAT_FEATURE_SPECTRALBAND_H_
#define KALDI_FEAT_FEATURE_SPECTRALBAND_H_
...
/// SpectralbandOptions contains basic options for computing spectralband features
...
struct SpectralbandOptions {
  ...
  SpectralbandOptions() : mel_opts(23),
                  // defaults the #mel-banks to 23 for the spectralband computations.
  ...
  void Register(OptionsItf *po) {
    ...
    po->Register("num-ceps", &num_ceps,
                 "Number of cepstra in spectralband computation (including C0)");
    po->Register("use-energy", &use_energy,
                 "Use energy (not C0) in spectralband computation");
    po->Register("energy-floor", &energy_floor,
                 "Floor on energy (absolute, not relative) in spectralband computation");
    po->Register("raw-energy", &raw_energy,
                 "If true, compute energy before preemphasis and windowing");
    po->Register("cepstral-lifter", &cepstral_lifter,
                 "Constant that controls scaling of spectralbands");
...
/// Class for computing spectralband features; see \ref feat_spectralband for more information.
class Spectralband {
 public:
  explicit Spectralband(const SpectralbandOptions &opts);
  ~Spectralband();
 ...
 private:
  ...
  KALDI_DISALLOW_COPY_AND_ASSIGN(Spectralband);
};
...
#endif  // KALDI_FEAT_FEATURE_spectralband_H_
