...
num_tgt=100        # force to use number of outputs in the MLP (default is autodetect)
...
input_label="ark:ali-to-post ark:$labels ark:- |"
# CMVN:
...
steps/nnet/train_scheduler.sh \
  ...
  $mlp_init "$feats_tr" "$feats_cv" "$input_label" "$input_label" $dir || exit 1
