...
# Config:
prefix=$1
gmmdir=$prefix/exp/tri3
data_fmllr=$prefix/data
...
if [ $stage -le 2 ]; then
  ...
  ali=${gmmdir}_ali
  labels=$data_fmllr/train/label.pdf
  ...
  $cuda_cmd $dir/log/train_nnet.log \
    ...
    $data_fmllr/train_tr90 $data_fmllr/train_cv10 $labels $dir || exit 1;
...
dir=$prefix/exp/dnn4_pretrain-dbn_dnn_smbr
srcdir=$prefix/exp/dnn4_pretrain-dbn_dnn
acwt=0.2
...
if [ $stage -le 3 ]; then
  ...
  steps/nnet/make_denlats.sh --nj 20 --cmd "$decode_cmd" --acwt $acwt \
    ...
    $data_fmllr/train $prefix/data/lang $srcdir ${srcdir}_denlats || exit 1;
fi
